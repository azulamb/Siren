...
window.addEventListener('DOMContentLoaded', ( event ) =>
{
	...
	customElements.define( 'tweet-button', class extends HTMLElement implements TweetButtonElement
	{
		...
		private update()
		{
			...
			if ( this.url ) { params.append( 'url', this.url ); }
			const hashtags = this.hashtags();
			if ( 0 < hashtags.length ) { params.append( 'hashtags', hashtags.join( ',' ) ); }
			if ( this.via ) { params.append( 'via', this.via ); }
			if ( this.in_reply_to ) { params.append( 'in_reply_to', this.in_reply_to ); }
			const related = this.related();
			if ( 0 < related.length ) { params.append( 'related', related.join( ',' ) ); }
		...
		private twitterArray( data: string )
		{
		...
		public hashtags( ... values: string[] ): void;
		hashtags( ... values: string[] )
		{
			if ( values.length === 0 ) { return this.twitterArray( this.getAttribute( 'hashtags' ) || '' ); }
		...
		public related( user: string ): void;
		public related( user1: string, user2: string ): void;
		related( user1?: string, user2?: string )
		{
			if ( user1 === undefined ) { return this.twitterArray( this.getAttribute( 'related' ) || '' ); }
			const users: string[] = [];
			if ( user1 ) { users.push( user1.replace( /^\@+/, '' ) ); }
			if ( user2 ) { users.push( user2.replace( /^\@+/, '' ) ); }
